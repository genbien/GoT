...
d3.json('data.json', function(err, response){
	...
	// Draw the scenes
	var scenes = svg.selectAll('.scene').data(narrative.scenes()).enter()

	// scene caption
	var cap = scenes.append('g').attr('class', 'line')
	...
	// 		.attr('x2', 0)
	// 		.attr('y2', height)
	// 		.attr('style', 'stroke:rgb(175,175,175);stroke-width:1');
	cap.append('rect')
			.attr('width', 41)
			.attr('height', height)
			.attr('x', 0)
			.attr('y', 0)
			.attr('style', function(scene, id) {
				return [
					'stroke:none',
					'fill:'+response.colors[id]
				].join(';');
			})


	// cap.append('text').text('Winterfell')
	// 	.attr('x', 5)
	// 	.attr('y', 10)
	// 	.attr('style', 'fill:red')


	scenes.append('g').attr('class', 'scene')
		.attr('transform', function(d){
				var x,y;
